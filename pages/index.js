...
export default function Tracker() {
  ...
  const [meta,setMeta]=useState("Click sync to load");
  ...
  async function fetchDeals(){
    setLoading(true);setError(null);setDeals([]);
    try{
      ...
      setDeals(mapped);
      const active=mapped.filter(d=>!["10. Disqualified","9. Disengaged"].includes(d.stage)).length;
      setMeta(`${mapped.length} deals · ${active} active · ${new Date().toLocaleDateString("en-GB",{day:"2-digit",month:"short",year:"numeric"})}`);
    }catch(e){setError(e.message);setMeta("Sync failed");}
  ...
  return(<>
    ...
    <div className="header">
      ...
      <div style={{display:'flex',gap:8,alignItems:'center'}}>
        <span style={{fontSize:11,color:'#999',marginRight:4}}>{meta}</span>
        <button onClick={exportCSV} disabled={!filtered.length}>↓ Export CSV</button>
        <button className="sync-btn" onClick={fetchDeals} disabled={loading}>{loading?"Syncing...":"↻ Sync HubSpot"}</button>
      </div>
    </div>
    <div className="main">
      <div className="sbar">
      ...
      {error&&<div className="err"><strong>Could not load from HubSpot</strong> — {error}. <a style={{cursor:'pointer',textDecoration:'underline'}} onClick={fetchDeals}>Try again</a></div>}
      {loading?<div>{[...Array(8)].map((_,i)=><div key={i} className="skel" style={{animationDelay:`${i*.1}s`}}/>)}</div>
      :filtered.length===0?<div className="empty">{deals.length===0?'Click "Sync HubSpot" to load your deals.':"No deals match your filters."}</div>
      :<><div className="tw"><table><colgroup>{cols.map(c=><col key={c.k} style={{width:c.w}}/>)}</colgroup><thead><tr>{cols.map(c=><th key={c.k} onClick={()=>handleSort(c.k)}>{c.l} {sortKey===c.k?(sortDir==="asc"?"↑":"↓"):"⇅"}</th>)}</tr></thead><tbody>{filtered.map((d,i)=>{const sc=SS[d.stage]||{bg:"#eee",t:"#444",d:"#999"};const pc=PS[d.pipeline]||{bg:"#eee",t:"#444"};return<tr key={d.id||i}><td className="cp"><a className="lnk" href={`https://app.hubspot.com/contacts/22310328/record/0-3/${d.id}`} target="_blank" rel="noreferrer">↗</a>{d.company}</td><td><span className="pb" style={{background:pc.bg,color:pc.t}}>{d.pipeline==="World ID Integrations"?"Integrations":"Rewards"}</span></td><td className="cm" title={d.use_case||""}>{d.use_case||"—"}</td><td className="cm">{d.ownerLabel}</td><td><span className="bdg" style={{background:sc.bg,color:sc.t}}><span className="dot" style={{background:sc.d}}/>{d.stage}</span></td><td className="cm">{d.country||"—"}</td><td className="ch">{d.region||"—"}</td><td className="ch">{d.lastActivity||"—"}</td><td className="cn" title={d.description||""}>{d.description||"—"}</td></tr>;})}</tbody></table></div><p className="tally">Showing {filtered.length} of {deals.length} deals</p></>}
    </div>
  </>);
}
